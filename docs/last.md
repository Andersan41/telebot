...
```
 ...
 3. ADX flat filter        — ADX < 25 (18 для compression) → NO_SIGNAL
 ...
 7. Compression breakout   — strong trigger + ADX>=25 + vol>=2x + ST aligned
 8. Trigger gate           — trigger_required=true + нет триггера → NO_SIGNAL
 9. Momentum entry mode    — fallback: ST aligned + EMA aligned + ADX>=25 + volume
...
13. Build reasons          — score = count(reasons)
 14. Min score gate         — score < min_score_for_signal (4) → NO_SIGNAL
...
| `CONFIDENCE_STRONG_THRESHOLD` | `70` | Порог strong quality в confidence_v2 |
| `CONFIDENCE_MODERATE_THRESHOLD` | `40` | Порог moderate quality в confidence_v2 |
| `MIN_SCORE_FOR_SIGNAL` | `4` | Минимум count-условий для сигнала |

> `QUALITY_STRONG/MODERATE_THRESHOLD` удалены как dead code — quality label вычисляется через `CONFIDENCE_*` пороги в `confidence_v2._quality_label()`.
...
| Trend | 4 | ADX ≥ 25 + EMA spread rising |
| Fallback | 5 | Range |

**Compression → Breakout ADX gap:**
- Compression detected by ATR% (не ADX), поэтому ADX 18-24 при compression — "серая зона"
- ADX filter ослаблен: `effective_adx_min=18` (вместо 25) — пропускает
- Breakout mode требует `ADX ≥ 25` — блокирует
- Итого: ADX 18-24 при compression → NO_SIGNAL (проходит ADX filter, но не проходит breakout check)
- Это **by design**: цена расширяется, но breakout не подтверждён
...
- **Weak:** trade only if `risk_weak_trade=true` (false)
  - `MIN_SCORE_FOR_SIGNAL=4` отсекает VERY WEAK (score<2) и часть WEAK (score 2-3)
  - `RISK_WEAK_TRADE=false` блокирует оставшиеся WEAK (score 4-5) на уровне risk
  - Два последовательных фильтра: signal_engine → dynamic_risk
...
8. **Compression breakout mode (M7)** — вместо hard block на compression, можно пускать breakout при strong trigger + ADX≥25 + volume≥2x + ST aligned.
