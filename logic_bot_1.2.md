# ЛОГИКА БОТА v1.2.0 — Полная карта торгового сигнала

> **Версия**: v1.2.0 | **Стратегия**: v2.5.0 | **Стек**: Python 3.11, ccxt, pandas-ta, SQLAlchemy async, python-telegram-bot 20.x
> **База**: SQLite (aiosqlite) | **Расписание**: APScheduler (каждые 15 мин)
> **Вход**: `main.py` → `TaskScheduler` → `run_scan_cycle()` → `scan_symbol_v2()`
> **Аудит**: Проверено по исходному коду. ~32 gate checks, 46 reason codes, 72 features.
> **Config Version**: `_CONFIG_VERSION = 3`
...
10. [Фаза 1.6: Entry Trigger (первый)](#10-фаза-16-entry-trigger)
11. [Фаза 1.7: LTF Confirmation](#11-фаза-17-ltf-confirmation)
12. [Analytical Overlays (влияют на sizing)](#12-analytical-overlays)
13. [Фаза 2: Feature Builder](#13-фаза-2-feature-builder)
14. [Фаза 3: Probability Engine](#14-фаза-3-probability-engine)
15. [Фаза 4: Risk Engine](#15-фаза-4-risk-engine)
16. [Фаза 4.5: Entry Trigger (второй)](#16-фаза-45-entry-trigger)
17. [Фаза 5–6: Build SignalResult + Dedup](#17-фаза-56-build-signalsresult--dedup)
18. [Фаза 7: Execution Filters + TOCTOU Recheck](#18-фаза-7-execution-filters)
19. [Фаза 8: Save + Notify](#19-фаза-8-save--notify)
20. [Полная карта фильтров (последовательных и параллельных)](#20-полная-карта-фильтров)
21. [Веса параметров и скоринг](#21-веса-параметров-и-скоринг)
22. [Система аудита и логирования](#22-система-аудита-и-логирования)
23. [Cooldown и Dedup логика](#23-cooldown-и-dedup-логика)
24. [Post-signal: Управление позицией](#24-post-signal-управление-позицией)
25. [Контекстные источники данных](#25-контекстные-источники-данных)
26. [Конфигурация (полный реестр параметров)](#26-конфигурация)
27. [Аудит-коды (reason codes) — полный реестр](#27-аудит-коды)
28. [Граф потока данных](#28-граф-потока-данных)
29. [Новые модули v2.5](#29-новые-модули)
30. [Известные пробелы и рекомендации](#30-известные-пробелы)
31. [Changelog v1.2](#31-changelog)
...
|--------|------|-------|------------|
| Pattern Engine | `strategy/pattern_engine.py` | 500 | ICT паттерны: sweep→displacement→MSS / trend→BOS |
| Feature Builder | `strategy/feature_builder.py` | 532 | 72 признаков в плоский вектор (48 ML keys) |
| Probability Engine | `strategy/probability_engine.py` | 486 | P(TP), expected RR, profit factor |
| Risk Engine | `risk/engine.py` | 298 | Hard gates + Kelly sizing |
| Trade Engine | `strategy/trade_engine.py` | 324 | SL/TP на основе ликвидности |
| Scanner | `scheduler/scanner.py` | 2025 | Весь пайплайн scan_symbol_v2 |
| Config | `config/settings.py` | 926 | 250+ параметров через .env |
| Database | `storage/database.py` | ~1400 | SQLAlchemy async, 10+ таблиц |
| Audit Reasons | `storage/audit_reasons.py` | 71 | 46 reason codes |
| Trace Builder | `storage/trace.py` | 261 | DecisionTrace, gate_path, features_snapshot |
| Context Fetcher | `context/fetcher.py` | 352 | 8 источников контекста |
| Indicators | `indicators/engine.py` | 201 | pandas-ta расчёты |
| Market Structure | `market_structure/structure.py` | 657 | Swing, BOS, CHoCH, MSS |
| HTF Bias V2 | `market_structure/htf_bias_v2.py` | 146 | W1→D1→H4→H1 voting |
| Breakout Quality | `liquidity/breakout_quality.py` | 167 | AMD stop-hunt vs real breakout |
| SMT Divergence | `derivatives/smt_divergence.py` | 191 | BTC vs alts divergence |
| Outcome Tracker | `scheduler/outcome_tracker.py` | 642 | SL/TP мониторинг, position management |
| Position Manager | `risk/position_manager.py` | 417 | Partial close, trailing, breakeven |
| Daily Limits | `risk/daily_limits.py` | 145 | 5 дневных условий |
| Circuit Breaker | `scheduler/circuit_breaker.py` | 71 | 3 SL → пауза 30 мин |

**Новые модули (v2.5):**

| Модуль | Файл | Строк | Назначение |
|--------|------|-------|------------|
| Market Phase Engine | `strategy/market_phase_engine.py` | 411 | 8 рыночных фаз (Accumulation, Distribution, etc.) |
| Market Thesis Engine | `strategy/market_thesis_engine.py` | 2479 | Dynamic DAG для рыночных сценариев |
| Scenario Engine | `strategy/scenario_engine.py` | 341 | Конкурирующие BUY/SELL сценарии |
| Scenario Memory | `strategy/scenario_memory.py` | 298 | Expected vs Observed статистика |
| Decision Engine | `strategy/decision_engine.py` | 169 | Выбор лучшей гипотезы |
| Hypothesis Engine | `strategy/hypothesis.py` | 244 | NarrativeType, Hypothesis, HypothesisSet |
| Entry Trigger | `strategy/entry_trigger.py` | 135 | Отдельный триггер входа |
| Confirmation Engine | `strategy/confirmation_engine.py` | 229 | LTF подтверждение (MICRO_BOS, ZONE_RETEST, MOMENTUM) |
| Trade Plan | `strategy/trade_plan.py` | 133 | TradePlan dataclass + TargetScore |
| Weight Manager | `strategy/weight_manager.py` | 88 | Статистическая корректировка Probability Engine |
| Volume Profile | `liquidity/volume_profile.py` | 186 | POC/VAH/VAL |
| OB State Tracker | `liquidity/ob_state.py` | 298 | State machine для OB/FVG mitigation |
| Elliott Wave | `elliott_wave/analysis.py` | 389 | Wave analysis |
| Analytics (17 модулей) | `analytics/*.py` | — | Full report, performance, gate funnel, evolution |
| ML (8 модулей) | `ml/*.py` | — | Training, dataset, triple barrier, auto-retrain |
...
**Файл**: `scheduler/scanner.py`, строки 280–2025
...
Phase 1.41: Breakout Quality                       ← условный hard gate
Phase 1.42: OB Retest + Mitigation                 ← условный hard gate
Phase 1.43: Session Filter (Kill Zones)            ← условный hard gate
Phase 1.44: SMT Divergence                         ← soft (не блокирует)
Phase 1.45: HTF Bias V2                            ← hard gate
Phase 1.5:  HTF POI + Trade Plan (SL/TP)           ← последовательные
Phase 1.6:  Entry Trigger (первый)                  ← последовательный
...
**Строки 438–548.** Чистые вычисления, последовательные проверки.
...
**Строки 554–641.** Последовательные.
...
### 1.41 Breakout Quality (строки 673–734)

**Модуль**: `liquidity/breakout_quality.py` (167 строки)
...
### 1.42 OB Retest + Mitigation (строки 736–831)
...
### 1.43 Session Filter / Kill Zones (строки 835–853)
...
### 1.44 SMT Divergence (строки 661–671)

**Модуль**: `derivatives/smt_divergence.py` (191 строка)
...
### 1.45 HTF Bias V2 (строки 854–1078)

**Модуль**: `market_structure/htf_bias_v2.py` (146 строк)
...
- 1 directional + 2 neutral → WEAK (single-TF conviction)
- W1 согласен с D1 или H4 → WEAK (pair agreement)
- Нет соглашения → NEUTRAL

**Confidence** — EMA-spread based, informational only (НЕ statistical confidence):
```
...
**Модуль**: `strategy/htf_poi.py` (173 строки)
...
## 10. Фаза 1.6: Entry Trigger (первый)
...
## 11. Фаза 1.7: LTF Confirmation

**Строки 1147–1217.** Опциональный.

**Если `config.trading.scan_mode="multi_tf"` и `config.trading.confirm_tf_enabled=True:`**

**Модуль**: `strategy/confirmation_engine.py` (229 строк)

1. Загрузка свечей подтверждения (например, 5m)
2. Расчёт индикаторов на подтверждающем TF
3. `find_confirmation()` — проверка подтверждения на LTF с тремя методами:
   - **MICRO_BOS**: Микро-break of structure на LTF
   - **ZONE_RETEST**: Возврат к зоне entry на LTF
   - **MOMENTUM**: Импульсное движение в направлении entry
4. Если подтверждено → entry_price корректируется с LTF
5. Если нет → `LTF_NO_CONFIRMATION` / `LTF_DATA_UNAVAILABLE`

---

## 12. Analytical Overlays (НЕ блокируют, НО влияют на sizing)

**Строки 1219–1523.** Работают параллельно, записывают результаты, но **никогда не возвращают None**. Их выходы (`_thesis_score`, `_thesis_stability`) передаются в Risk Engine как множители размера позиции — это **активные overlay**, а не shadow.

### Market Phase Detection (строки 1219–1251)
**Модуль**: `strategy/market_phase_engine.py` (411 строк)

`MarketPhaseEngine.assess()` — определяет 8 рыночных фаз:
1. Accumulation
2. Markup
3. Distribution
4. Markdown
...
### Market Thesis Engine (строки 1253–1425)
**Модуль**: `strategy/market_thesis_engine.py` (2479 строк)
...
### Hypothesis + Decision Engine (строки 1427–1523)
**Модули**: `strategy/hypothesis.py` (244), `strategy/decision_engine.py` (169)

- Строит `HypothesisSet`
- Оценивает через `DecisionEngine.decide()`
- Записывает expected metrics в `ScenarioMemory`

### Scenario Engine (строки 1634–1697)
**Модуль**: `strategy/scenario_engine.py` (341 строка)

- Генерирует конкурирующие сценарии из `LiquidityGraph`
- Каждый сценарий = narrative + expected direction + confidence

### Weight Manager (строка 88)
**Модуль**: `strategy/weight_manager.py`

- Корректирует выводы Probability Engine на основе кумулятивной статистики из `ScenarioMemory`

---

## 13. Фаза 2: Feature Builder

**Строки 1525–1632.** Чистые вычисления (не фильтр).
...
### SetupFeatures (72 fields)

| Категория | Признаки |
|-----------|----------|
| **Setup Classification (1)** | `setup_type` (reversal/continuation) |
| **ICT Pattern (6)** | `has_bos`, `has_sweep`, `has_ob`, `has_fvg`, `has_displacement`, `components_count` |
| **Reversal Metrics (5)** | `has_mss`, `mss_score`, `mss_causality`, `displacement_atr_ratio`, `sweep_to_mss_bars` |
| **Pattern Metrics (4)** | `ob_distance_pct`, `fvg_size_pct`, `sweep_reclaim_speed`, `sweep_strength` |
| **Market Structure (2)** | `structure_trend`, `structure_bos_aligned` |
| **Volume (3)** | `volume_ratio`, `volume_delta_pct`, `volume_above_avg` |
| **Volume Profile (3)** | `vp_poc_distance_pct`, `vp_in_value_area`, `vp_price_range_pct` |
| **Volatility (3)** | `atr`, `atr_pct`, `regime` |
...
| **Execution (4)** | `session`, `candle_close_pct`, `is_reversal`, `entry_armed` |
| **Soft Multipliers (5)** | `htf_alignment_score`, `premium_discount_score`, `htf_bias_penalty`, `ob_state_multiplier`, `smt_divergence_score` |
| **Elliott Wave (5)** | `wave_confidence`, `wave_direction`, `wave_conflict`, `wave_alternatives_count`, `wave_primary_label` |
| **S/R Proximity (2)** | `nearest_support_pct`, `nearest_resistance_pct` |
| **Visual Data (12)** | `sweep_price`, `sweep_candle_timestamp`, `displacement_candle_timestamp`, `mss_level_price`, `mss_candle_timestamp`, `ob_high_price`, `ob_low_price`, `ob_candle_timestamp`, `fvg_top_price`, `fvg_bottom_price`, `fvg_candle_timestamp`, `bos_level_price`, `bos_candle_timestamp` |

### to_vector() — для ML
Конвертация в плоский dict: categorical encoding (setup_type ±1, structure_trend ±1/0, regime 1/0.5/−0.5/−1, session 0–4), booleans → ints. **48 numeric keys** для ML input.

### Full Feature Vector в Trace
`DecisionTraceBuilder.set_features()` сохраняет:
- `_features` — отфильтрованные поля (для DB и быстрого доступа)
- `_full_feature_vector` — все 72 признака (для ML replay и переобучения)
...
### 14.1 Rules-Based Estimation (`_predict_rules`)
...
**Expected net R**: `expected_net_r = p * b - (1-p)`, где `p = winrate/100`, `b = rr_ratio`

**Profit Factor**: `model_pf = p * b / max(1-p, 0.01)`

### 14.2 ML-Based Estimation (`_predict_ml`)
...
| 9 | SL min ATR multiplier | `SL_ATR_CONFLICT` | `sl_distance_pct < atr_pct × sl_min_atr_multiplier` (2.0). Relax floor если ATR-min > dynamic max |

### 15.2 Position Sizing:

#### EV Gate (applies to BOTH fixed and Kelly):
```python
p = probability.p_tp
b = rr_ratio
...
### 15.3 Мультипликативные корректировки (после Kelly/fixed):
...
## 16. Фаза 4.5: Entry Trigger (второй)
...
Вторая проверка Entry Trigger **после** Risk Engine — гарантирует, что цена всё ещё в зоне входа.

---

## 17. Фаза 5–6: Build SignalResult + Dedup

### Phase 5: Build SignalResult (строки 1867–1912)
...
### Phase 6: Dedup (строки 1914–1993)
...
## 18. Фаза 7: Execution Filters + TOCTOU Recheck

**Строки 1995–2108.** Последовательные.
...
| 4 | **TOCTOU Recheck** | `PORTFOLIO_MAX_ACTIVE` / `PORTFOLIO_MAX_RISK` | Повторная проверка portfolio limits перед финализацией. Атомарный резерв. Защита от race condition между asyncio tasks. |
| 5 | **Pre-reserve Daily Limits** | `DAILY_LIMIT_HIT` | Атомарная проверка + резерв дневных лимитов перед сохранением |

---

## 19. Фаза 8: Save + Notify
...
[4.0] Risk Engine: all hard gates (R:R, SL limits, min notional)
  ↓
[4.5] Entry Trigger (second check)
  ↓
[6.0] Dedup (cooldown)
  ↓
...
[Soft] Volume Profile               → feature (POC distance, VA)
```

---

## 21. Веса параметров и скоринг

### Feature Builder: Веса признаков (для ML input)

Признаки не имеют явных весов в Feature Builder — все 72 features передаются как есть. **48 numeric keys** для ML-модели. Веса определяются ML-моделью (XGBoost/RandomForest) или rules-based fallback.
...
## 22. Система аудита и логирования

### DecisionTraceBuilder (`storage/trace.py`, 261 строк)
...
| `_features` | dict | Feature snapshot (filtered fields) |
| `_full_feature_vector` | dict | Full 72-feature vector (for ML replay) |
...
### Signal Audit Log (`storage/audit_reasons.py`, 71 строк)
...
## 24. Post-signal: Управление позицией

### Outcome Tracker (`scheduler/outcome_tracker.py`, 642 строки)
...
### Exit Triggers (порядок приоритета, `risk/position_manager.py`, 417 строк)
...
### ExitPlan (связь TradeEngine ↔ PositionManager)
...
### Partial Close Logic (через ExitPlan)
```
...
## 25. Контекстные источники данных

### ContextFetcher (`context/fetcher.py`, 352 строки)
...
### Indicators (`indicators/engine.py`, 201 строк)
...
### Market Structure (`market_structure/structure.py`, 657 строк)
...
## 26. Конфигурация (полный реестр параметров)

### Конфигурационные классы: 14
### Общее количество параметров: ~250+

#### TradingConfig (~55 params)
...
#### AppConfig Feature Flags (~35 params)
...
## 27. Аудит-коды (reason codes) — полный реестр
...
│
├─ Phase 4.5: Entry Trigger (second) ─────────────────────── BLOCKED?
│
...
│  └─ Pre-reserve Daily Limits ───────────────────────────── BLOCKED?
│
└─ Phase 8: Save + Notify
   ├─ db.save_signal()
   ├─ trace.save()
...
## 29. Новые модули v2.5

### Strategy Modules

| Модуль | Файл | Строк | Назначение |
|--------|------|-------|------------|
| Market Phase Engine | `strategy/market_phase_engine.py` | 411 | 8 рыночных фаз (Accumulation, Distribution, Expansion, Compression, Trend, Range, Markup, Markdown) |
| Market Thesis Engine | `strategy/market_thesis_engine.py` | 2479 | Dynamic DAG для рыночных сценариев с жизненным циклом нод |
| Scenario Engine | `strategy/scenario_engine.py` | 341 | Генерация конкурирующих BUY/SELL сценариев |
| Scenario Memory | `strategy/scenario_memory.py` | 298 | Expected vs Observed статистика по narrative |
| Decision Engine | `strategy/decision_engine.py` | 169 | Выбор лучшей гипотезы из HypothesisSet |
| Hypothesis Engine | `strategy/hypothesis.py` | 244 | NarrativeType, Hypothesis, HypothesisSet |
| Entry Trigger | `strategy/entry_trigger.py` | 135 | Отдельный триггер входа (2 проверки) |
| Confirmation Engine | `strategy/confirmation_engine.py` | 229 | LTF подтверждение: MICRO_BOS, ZONE_RETEST, MOMENTUM |
| Trade Plan | `strategy/trade_plan.py` | 133 | TradePlan dataclass + TargetScore |
| Trade Thesis | `strategy/trade_thesis.py` | 344 | TradeThesisManager |
| Weight Manager | `strategy/weight_manager.py` | 88 | Статистическая корректировка Probability Engine |
| Pattern Engine Config | `strategy/pattern_engine.py` | — | BOS/sweep requirement, OB proximity |
...
| Volume Profile | `liquidity/volume_profile.py` | 186 | POC/VAH/VAL |
| OB State Tracker | `liquidity/ob_state.py` | 298 | State machine для OB/FVG mitigation |
| Pool | `liquidity/pool.py` | 246 | Unified liquidity landscape |
| Equal Levels | `liquidity/equal_levels.py` | 159 | Equal Highs/Lows detection |
| External Liquidity | `liquidity/external.py` | 199 | External liquidity (old highs/lows) |

### Risk Modules

| Модуль | Назначение |
|--------|------------|
| `risk/market_regime.py` | Market regime detection (Trend, Range, Compression, Expansion, Reversal) |
| `risk/volatility_regime.py` | Volatility regime classification |
| `risk/no_trade_zones.py` | No-trade zone checking |
| `risk/news_filter.py` | News event filtering (stub) |
| `risk/dynamic_risk.py` | Dynamic risk adjustment |
| `risk/metrics.py` | Risk metrics calculation |

### Elliott Wave

| Модуль | Файл | Строк | Назначение |
|--------|------|-------|------------|
| Analysis | `elliott_wave/analysis.py` | 389 | Wave analysis |
| Wave Types | `elliott_wave/wave_types.py` | — | Wave type definitions |

### Analytics (17 модулей)

| Модуль | Назначение |
|--------|------------|
| `analytics/full_report.py` | Full analytics report |
| `analytics/performance.py` | Performance analytics |
| `analytics/gate_funnel.py` | Gate funnel analysis |
| `analytics/evolution.py` | Strategy evolution tracking |
| `analytics/counterfactual.py` | Counterfactual analysis |
| `analytics/calibration.py` | Probability calibration |
| `analytics/feature_drift.py` | Feature drift detection |
| `analytics/entry_delay.py` | Entry delay analysis |
| `analytics/drift_report.py` | Drift report generation |
| `analytics/daily_report.py` | Daily report |
| `analytics/distributions.py` | Distribution analysis |
| `analytics/factor_stats.py` | Factor statistics |
| `analytics/sl_tp_analysis.py` | SL/TP analysis |
| `analytics/trace_export.py` | Trace export |
| `analytics/trades_export.py` | Trades export |
| `analytics/version_compare.py` | Version comparison |
...
| `ml/build_dataset.py` | Dataset building |
| `ml/triple_barrier.py` | Triple barrier labeling |
| `ml/validate_oos.py` | Out-of-sample validation |
| `ml/auto_retrain.py` | Automatic retraining |
| `ml/ab_compare.py` | A/B comparison |
| `ml/config.py` | ML configuration |
| `ml/cache_ohlcv.py` | OHLCV caching |

---

## 30. Известные пробелы и рекомендации
...
3. **Shadow Mode Engines**: Market Thesis Engine (2479 строк) и Scenario Engine (341 строка) — сложные, но не влияют на сигналы. Их данные используются только для ML-обучения и аналитики. Рекомендация: проверить, что Scenario Memory确实 накапливает данные для ML.
...
12. **Volume Profile**: Добавлен в Feature Builder, но POC/VAH/VAL расчёт может быть неточным на малых lookback. Рекомендация: валидировать на历史数据.
...
- **Analytical Overlays** — не блокируют, но `_thesis_score` и `_thesis_stability` влияют на sizing через Risk Engine. При модификации пайплана проверять, что features snapshot всё ещё корректен. Не путать с true shadow (который действительно не влияет).
...
- **TOCTOU Protection**: Phase 7 включает атомарный резерв (portfolio recheck + daily limits pre-reserve) для предотвращения race condition между asyncio tasks.
...
- **Entry Trigger Dual-Check**: Проверяется дважды (до и после Risk Engine) для гарантии, что цена в зоне входа.
- **Config Version**: `_CONFIG_VERSION = 3` в `scheduler/scanner.py`. Бампать при изменении任何 config threshold.

---

## 31. Changelog

### v1.2.0 (2026-09-10) — Полный аудит v2.5.0

**Обновления документации:**
- Обновлены строковые счётчики по всем модулям (scanner: 2025, feature_builder: 532, probability_engine: 486, risk/engine: 298, trade_engine: 324, position_manager: 417, outcome_tracker: 642)
- Feature Builder: обновлено с ~53 до **72 fields** (48 ML vector keys)
- Gate checks: обновлено с ~30 до **32 blocking points**
- Config parameters: обновлено с ~45+ до **~250+ total** (14 config classes)
- Pipeline phases: обновлено с ~18 до **22 phases**
- Добавлены новые модули: Market Phase Engine, Market Thesis Engine, Scenario Engine, Scenario Memory, Decision Engine, Hypothesis Engine, Entry Trigger, Confirmation Engine, Trade Plan, Weight Manager, Volume Profile, OB State Tracker, Elliott Wave
- Добавлены analytics модули (17 модулей)
- Добавлены ML модули (8 модулей)
- Добавлена секция 29: Новые модули v2.5
- Обновлён граф потока данных (с new phases)
- Config version: `_CONFIG_VERSION = 3`

**Структурные изменения пайплайна:**
- Phase 1.42: Confirmation Score Gate (weighted: BOS=2, FVG=1, OB=1)
- Phase 1.42: OB Retest + Mitigation (7 sub-checks)
- Phase 1.43: Session Filter (Kill Zones)
- Phase 1.44: SMT Divergence (soft feature)
- Phase 1.55: Market Phase Detection (shadow)
- Phase 1.6: Market Thesis Engine (shadow)
- Phase 1.65: Scenario Engine (shadow)
- Phase 1.7: Hypothesis + Decision Engine (shadow)
- Phase 1.6: Entry Trigger (first check)
- Phase 4.5: Entry Trigger (second check)
- Phase 7.5: Pre-reserve Daily Limits (atomic)
