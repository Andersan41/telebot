...
- Хранит `COINGECKO_SYMBOL_MAP` как сырую строку в `coingecko_symbol_map_str`
  (`settings.py:81`); парсинг в словарь — **lazy**, через `@property coingecko_symbol_map`
  (`settings.py:91-93`), считается каждый раз при обращении.
...
**Параметры индикаторов (захардкожены в коде, не env):**
`ema_fast/slow/trend`, `rsi_period/overbought/oversold/bull_min/bear_max`,
`macd_fast/slow/signal`, `adx_period/min`, `atr_period/multiplier_sl/multiplier_tp`,
`supertrend_period/multiplier`, `volume_factor`, `candles_limit` — все заданы значениями
в `TradingConfig` (`config/settings.py:40-61`). Изменение требует правки кода;
перевод на env — задача [`plan/improvements/A1-indicators-env.md`](improvements/A1-indicators-env.md).
